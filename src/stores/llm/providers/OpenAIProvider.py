...
class OpenAIProvider(LLMInterface):
    ...
    def __init__(self, api_key: str, api_url: str = None, default_input_max_tokens: int = 1000
    ...
    def generate_text(self, prompt: str, chat_history : list = [], max_output_tokens: int = None, temp: float = None):
        
        if self.client is None:
            # raise ValueError("OpenAI client is not initialized. Please set the API key and URL.")
            self.logger.error("OpenAI client is not initialized. Please set the API key and URL.")
            return None
    ...
    def embed_text(self, text: str, document_type: str):
        ...
        if self.embedding_model is None:
            ...
            self.logger.error("Embedding model is not set. Please set the embedding model before embedding text.")
            return None
